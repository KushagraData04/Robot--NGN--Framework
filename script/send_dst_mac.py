...
def send_packet(n, iface):
    ...
    for i in range(n):
        ...
        dst_mac="22:33:33:33:33:44"
        packet = Ether(src=mac_address, dst=dst_mac)/ Dot1Q(vlan=vlan_id) / IP(dst=destination_ip) / ICMP()
        print(f"Sending packet {i+1} with MAC {mac_address} on interface {iface}")
