...
Simulate multiple neighbors via server
    ...
    Write    interface ${serverport1}; mode trunk; trunk allowed vlan add ${bvi_start_range}-${end_range}
    ...
    FOR  ${session}    IN RANGE   2    ${bvi_total_subif}
        ...
        Write    ipv4 address 10.66.${session}.1 prefix 24;
    ...
    FOR  ${session}    IN RANGE   2    ${bvi_total_subif}
        ...
        Write    sudo ${scripts_base_dir}/script/phy_subif_create.sh ${session} 10.66.${session}.2/${bvi_prefix} 10.66.${session}.1 ${server_iface1}
    ...
    FOR  ${session}    IN RANGE   2    ${bvi_total_subif}
        ...
        Write    sudo ${scripts_base_dir}/script/phy_subif_addip.sh ${session} 10.66.${session} ${bvi_prefix} ${bvi_ip_per_vlan} ${server_iface1}
        ${cli}=     Read Until Prompt
...
Cleanup
    ...
    FOR  ${session}    IN RANGE   2    ${bvi_total_subif}
        Write    top; bridge default;
        Write    vlan ${session}; irb disable;
        Run Keyword If    ${session} % 100 == 0    Commit Testcase
    END
    Commit Testcase
    Switch Connection    SERVER
