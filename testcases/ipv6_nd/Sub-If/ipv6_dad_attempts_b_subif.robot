...
Dad attempts in Dut1
    [Documentation]  Set Dad attempts 
    Switch Connection    ${DUT1.hostname}
    FOR    ${subif}    IN    @{DUT1.bundle_subif_interfaces}
        Write    interface subif ${subif.name}
        Write  ipv6 nd dad attempts ${DUT1.dad_attempts}
    END
    Commit Testcase

    #  Write     top;

Dad Attempts in DUT2
   [Documentation]  Dad Attempts 
   Switch Connection    ${DUT2.hostname}
    FOR    ${subif}    IN    @{DUT1.bundle_subif_interfaces}
   ${output}=   Execute Command  cat /proc/sys/net/ipv6/conf/${subif.name}/dad_transmits
        ${log_msg}=  Set Variable  dad attempts are: ${output}
        Log   ${log_msg}
        Should Contain  ${log_msg}   dad attempts are: ${DUT1.dad_attempts}
   Exit For Loop
    END
