...
Base Reachable Time on Bundle Subinterfaces
    [Documentation]    Set base reachable time on subinterfaces
    Switch Connection    ${DUT1.hostname}
...
Check RA Packets on DUT2 from BVI
    ...
    FOR    ${bvi}    IN    @{DUT1.bundle_bvi_interfaces}
        ${output}=   Execute Command  cat /proc/sys/net/ipv6/neigh/${bvi.name}/base_reachable_time_ms
