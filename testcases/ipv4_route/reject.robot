...
Reconfiguration DUT1
   ...
   Commit Testcase
   Write  router static ${static_ip} next-hop ${def_nh} action reject
   Commit Testcase

Ping Test
   Switch Connection  ${Server158.hostname}
#    Read Until Prompt
   Write    sudo ip netns exec ns-pega-228 ping 20.20.20.158 -c 10 > r_ping.txt
   Sleep    1s
   Write    ngn@8737
   Sleep    15s
   ...
   Login    ngn    ngn@8737
   Write    cat r_ping.txt
   ${cli}   Read Until Prompt
   Should Contain    ${cli}  Packet filtered 
   ...    100% packet loss
   Close Connection
