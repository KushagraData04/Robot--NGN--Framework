...
Configure Bundles and Ip on Bundle DUT2 bundle interfaces
  [Documentation]  Bundles Creation
...
Ping Test
  Switch Connection  ${Server158.hostname}
  Read Until Prompt
  Write    sudo ip netns exec ns-pega-228 ping 20.20.20.158 -c 10 > ping3.txt
  Sleep    1s
  Write    ngn@8737
  Sleep    12s
  Read Until Prompt

  Write    sudo ip netns exec ns-pega-228 ping6 229:20::158 -c 10 > ping6_3.txt
  Sleep    1s
  Write    ngn@8737
  Sleep    12s
  Read Until Prompt
  ...
  Login    ngn    ngn@8737
  Write    cat ping3.txt | grep "packet loss"
  ${cli}   Read Until Prompt
  Should Contain    ${cli}     0% packet loss

  Write    cat ping6_3.txt
  ${cli}   Read Until Prompt
  Should Contain    ${cli}     0% packet loss
  Close Connection
...
#   Read Until Prompt  # Ensures the command execution completes
#   Write    ngn@8737
#   Sleep  11s
  
#   Open Connection    ${Server}    alias=${Server158.hostname}    prompt=$    timeout=${DUT_TIMEOUT}    port=${DUT2.ssh_port}
#   Login    ngn    ngn@8737
#   Write    cat ping.txt | grep "packet loss"
#   ${cli}   Read Until Prompt
#   Should Contain    ${cli}     0% packet loss
#   Close Connection  # Closes the connection properly
