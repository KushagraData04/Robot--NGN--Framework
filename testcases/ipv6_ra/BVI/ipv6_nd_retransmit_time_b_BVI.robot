...
ND RA Retransmit Time in DUT1
    [Documentation]    Enable ND RA Retransmit Time 
    Switch Connection    ${DUT1.hostname}
    
    FOR    ${bvi}    IN    @{DUT1.bundle_bvi_interfaces}
        Write    interface subif ${bvi.name}
        Write  ipv6 nd ra-retransmit-time ${DUT1.retransmit_time}
    END
    Commit Testcase
...
Check RA Packets on DUT2 from BVI
    ...
    FOR    ${bvi}    IN    @{DUT1.bundle_bvi_interfaces}
        ...
        Log    ${output}
        Should Contain    ${output}     retrans timer ${DUT1.retransmit_time}ms
        Close Connection
        Exit For Loop
    END
