...
Enable Advertisement Interval on Bundle BVI
    [Documentation]    Enable ND advertisement interval
    Switch Connection    ${DUT1.hostname}
    
    FOR    ${bvi}    IN    @{DUT1.bundle_bvi_interfaces}
        Write    interface subif ${bvi.name}
        Write    ipv6 nd advertisement-interval enable
    END
    Commit Testcase
...
Check RA Packets on DUT2 from BVI
    ...
    FOR    ${bvi}    IN    @{DUT1.bundle_bvi_interfaces}
        ...
        Log    ${output}
        Should Contain    ${output}    advertisement interval option (7), length 8 (1):  600000ms 0x0000:  0000 0009 27c0
        Close Connection
        Exit For Loop
    END
