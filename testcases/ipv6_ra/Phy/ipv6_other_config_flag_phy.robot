...
Other Config Flag in DUT1
    [Documentation]  Enable other config flag
    FOR    ${phy}    IN    @{DUT1.phy_interfaces}
     ${cli}=  Write   interface physical ${phy.number} ; 
      Write  ipv6 nd other-config-flag enable
     END
     Commit Testcase
     Write     top;


Check other flag on DUT2
    ...
    Log    ${output}
    ${match}=    Evaluate    'Flags [other stateful]' in """${output}""" or 'Flags [managed, other stateful]' in """${output}"""
