...
All Permutation check
    ...
    lua state check    ${filepath}  ${hw_port} 
    Sleep    5s
    Add to bridge    ${port}
...
Testing on bundle
    ...
    Set Global Variable    ${port}       phy-1_3_1
    Set Global Variable    ${hw_port}    3967  #eport
    Set Global Variable    ${if_index}   17
    Set Global Variable    ${cli_phy}    1/3/1
    Switch Connection    lxc
    Append To File    path=${filepath}    content=Testing on falcon bundle with bundle ${bundle_id} with interface ${port} with if index ${if_index} and hw port ${hw_port}
    make interface    ${cli_phy}
